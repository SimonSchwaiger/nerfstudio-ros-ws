...
GRAPHICS_PLATFORM="${GRAPHICS_PLATFORM:-nvidia}"
PYTHONVER="${PYTHONVER:-3.8}"
DOCKER_RUN_ARGS="${DOCKER_RUN_ARGS:-"--net host"}"
BUILD_LOCAL="${BUILD_LOCAL:-false}"
ROS_DISTRO="${ROS_DISTRO:-"noetic"}" # Cheeky test for unified ros 1 and ros 2 script
...
if [ "$BUILD_LOCAL" == "false" ] && [ "$REGISTRY_AVAILABLE" == "true" ]; then
    ...
    echoerr "Nerfstudio is only installed in Dockerfile.remote. If you really intend to build locally, copy Nerfstudio installation over the local Dockerfile."
